...
class TestUserHandlers {
     ...
     */
    async createUsersInBatches(gender, city, count, requestId, batchSize) {
        const created = [];
        const errors = [];
        let failed = 0;
        for (let i = 0; i < count; i += batchSize) {
            const currentBatchSize = Math.min(batchSize, count - i);
            const batchPromises = Array.from({ length: currentBatchSize }, (_, j) => this.createTestUser(gender, city, i + j, requestId)
     ...
     */
    async createTestUser(gender, city, index, requestId) {
        ...
        const photos = Array.from({ length: photoCount }, (_, i) => `https://via.placeholder.com/400x600/4A90E2/FFFFFF?text=${gender === 'male' ? 'M' : 'F'}+${i + 1}`);
        let userRecord = null;
        try {
     ...
     */
    generateBio(interests, occupation, random) {
        const bios = [
...
exports.TestUserHandlers = TestUserHandlers;
//# sourceMappingURL=testUserHandlers.js.map