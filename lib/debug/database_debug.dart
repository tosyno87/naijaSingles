  ...
  static Future<void> checkDatabasePopulation() async {
    try {
      ...
      for (final doc in allUsersQuery.docs) {
        ...
        // Log user details
        debugPrint('👤 User ${doc.id}: age=${data['age']}, gender=${data['gender']}, lat=${data['latitude']}, lng=${data['longitude']}');
      }
      
      debugPrint('📈 Database Analysis:');
      debugPrint('   Users with age: $usersWithAge/${allUsersQuery.docs.length}');
      debugPrint('   Users with gender: $usersWithGender/${allUsersQuery.docs.length}');
      debugPrint('   Users with location: $usersWithLocation/${allUsersQuery.docs.length}');
      debugPrint('   Users in age range 18-35: $usersInAgeRange/${allUsersQuery.docs.length}');
  ...
  static Future<void> testUserQuery(String currentUserId) async {
    try {
      ...
      for (final doc in querySnapshot.docs) {
        final data = doc.data() as Map<String, dynamic>;
        debugPrint('👤 Found user: ${doc.id} (age: ${data['age']}, gender: ${data['gender']})');
      }
