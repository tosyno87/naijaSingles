  ...
  SearchUserBloc() : super(SearchuserInitial()) {
    on<LoadUserEvent>((event, emit) async {
      ...
      try {
        ...
        List<UserModel> userList = await DiscoveryService.getUsersForDiscovery(event.currentUser);
        
        // Get discovery stats for debugging
        final stats = await DiscoveryService.getDiscoveryStats(event.currentUser);
    ...
    on<CheckMigrationStatusEvent>((event, emit) async {
      try {
        final shouldPrompt = await DiscoveryService.shouldPromptForMigration(event.userId);
