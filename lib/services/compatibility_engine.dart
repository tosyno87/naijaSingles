...
class CompatibilityEngine {
  ...
  static double calculateCompatibility(UserModel user1, UserModel user2) {
    try {
      ...
      debugPrint('🎯 Compatibility ${user1.name} ↔ ${user2.name}: ${(totalScore * 100).toStringAsFixed(1)}%');
      debugPrint('   Age: ${(ageScore * 100).toStringAsFixed(1)}%, Location: ${(locationScore * 100).toStringAsFixed(1)}%, Interest: ${(interestScore * 100).toStringAsFixed(1)}%, Activity: ${(activityScore * 100).toStringAsFixed(1)}%, Completeness: ${(completenessScore * 100).toStringAsFixed(1)}%');
  ...
  static double _calculateAgeCompatibility(UserModel user1, UserModel user2) {
    try {
      ...
      if (ageDifference <= IDEAL_AGE_DIFFERENCE) {
        ...
        final score = 1.0 - ((ageDifference - IDEAL_AGE_DIFFERENCE) / (MAX_AGE_DIFFERENCE - IDEAL_AGE_DIFFERENCE)) * 0.7;
  ...
  static double _calculateLocationScore(UserModel user1, UserModel user2) {
    try {
      ...
      if (distanceKm <= 5.0) {
        ...
        final score = 1.0 - ((distanceKm - 5.0) / (MAX_DISTANCE_KM - 5.0)) * 0.8;
  ...
  static double _calculateInterestScore(UserModel user1, UserModel user2) {
    try {
      ...
      debugPrint('🎨 Interest matching: ${intersection.length} shared interests out of ${union.length} total');
  ...
  static Set<String> _extractInterests(UserModel user) {
    ...
    if (user.bio != null && user.bio!.isNotEmpty) {
      final bioWords = user.bio!.toLowerCase()
    ...
    if (user.address != null && user.address!.isNotEmpty) {
      final locationWords = user.address!.toLowerCase()
          .split(' ')
          .where((word) => word.length > 3)
          .toSet();
  ...
  static double _calculateUserActivity(UserModel user) {
    ...
    if (user.lastSeen != null) {
      ...
      if (daysSinceLastSeen <= 1) {
        ...
        activityScore += 0.3 - (daysSinceLastSeen / ACTIVITY_THRESHOLD_DAYS) * 0.2;
      }
    }
  ...
  static CompatibilityBreakdown getCompatibilityBreakdown(UserModel user1, UserModel user2) {
    final ageScore = _calculateAgeCompatibility(user1, user2);
    final locationScore = _calculateLocationScore(user1, user2);
    final interestScore = _calculateInterestScore(user1, user2);
    final activityScore = _calculateActivityScore(user1, user2);
...
class UserCompatibility {
  ...
  String get compatibilityPercentage => '${(compatibilityScore * 100).toStringAsFixed(1)}%';
  ...
  bool get isMediumCompatibility => compatibilityScore >= 0.5 && compatibilityScore < 0.7;
