...
class DiscoveryService {
  static final PrivacyMigrationService _migrationService = PrivacyMigrationService();
  
  /// Get user list for discovery (privacy-aware)
  static Future<List<UserModel>> getUsersForDiscovery(UserModel currentUser) async {
    try {
      ...
      final isMigrated = await _migrationService.isUserMigrated(currentUser.id!);
      ...
      if (isMigrated) {
        ...
        debugPrint('🔒 Privacy-aware discovery returned: ${users.length} users');
        return users;
      } else {
  ...
  static Future<List<UserModel>> getNearbyUsers(
    ...
    try {
      ...
      if (isMigrated) {
        ...
        return await PrivacyAwareUserSearchRepo.getUsersNearby(currentUser, radiusMiles);
      } else {
  ...
  static Future<List<UserModel>> getMatches(UserModel currentUser) async {
    try {
      ...
      if (isMigrated) {
        ...
        debugPrint('📋 Using traditional matches - loading from user subcollection');
  ...
  static Future<List<UserModel>> _getTraditionalMatches(UserModel currentUser) async {
    try {
      ...
      for (var doc in snapshot.docs) {
        try {
          ...
          final userDoc = await PrivacyAwareUserSearchRepo.docRef.doc(doc.id).get();
          if (userDoc.exists) {
  ...
  static Future<Map<String, dynamic>> getDiscoveryStats(UserModel currentUser) async {
    try {
      final isMigrated = await _migrationService.isUserMigrated(currentUser.id!);
  ...
  static Future<bool> migrateAndRefreshDiscovery(String userId) async {
    try {
      ...
      if (success) {
        debugPrint('✅ Migration successful, discovery will now use privacy-aware system');
        return true;
      } else {
    ...
    try {
      ...
      if (isMigrated) {
        ...
        if (filteredData != null) {
          return await PrivacyAwareUserSearchRepo.createUserModelFromFilteredData(filteredData, userId);
        }
      } else {
