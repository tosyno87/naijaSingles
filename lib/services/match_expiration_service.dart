...
class MatchExpirationService {
  ...
  static const Duration WARNING_THRESHOLD = Duration(days: 5); // Warn 2 days before expiry
  ...
  CollectionReference get _expiredMatchesCollection => _firestore.collection('expiredMatches');
  ...
  bool isMatchExpired(MatchModel match) {
    ...
    final hasMessages = match.lastMessageAt != null;
    
    // Don't expire matches with recent messages
    if (hasMessages) {
      final lastMessageAge = DateTime.now().difference(match.lastMessageAt!);
      if (lastMessageAge < MATCH_EXPIRY_DURATION) {
        return false;
      }
    }
  ...
  bool isMatchNearExpiry(MatchModel match) {
    ...
    // Don't warn for matches with recent messages
    if (hasMessages) {
      final lastMessageAge = DateTime.now().difference(match.lastMessageAt!);
      if (lastMessageAge < WARNING_THRESHOLD) {
        return false;
      }
    }
      ...
      try {
        final querySnapshot = await _matchesCollection
            ...
            .where('matchedAt', isLessThan: 
                Timestamp.fromDate(DateTime.now().subtract(MATCH_EXPIRY_DURATION)))
        ...
        debugPrint('📋 Found ${expiredMatches.length} expired matches for user $userId');
  ...
  Future<List<MatchModel>> getMatchesNearExpiry(String userId) async {
    return await PerformanceMonitor.measure('get_matches_near_expiry', () async {
      try {
        final querySnapshot = await _matchesCollection
            .where('users', arrayContains: userId)
            .where('matchedAt', isLessThan: 
                Timestamp.fromDate(DateTime.now().subtract(WARNING_THRESHOLD)))
            .where('matchedAt', isGreaterThan: 
                Timestamp.fromDate(DateTime.now().subtract(MATCH_EXPIRY_DURATION)))
            .get();
        ...
        debugPrint('⚠️ Found ${nearExpiryMatches.length} matches near expiry for user $userId');
    ...
    return await PerformanceMonitor.measure('archive_expired_match', () async {
      try {
        ...
        for (final userId in userIds) {
          ...
          batch.delete(_usersCollection.doc(userId).collection('Matches').doc(otherUserId));
        }
        ...
        if (chatThreadId != null) {
          ...
          if (chatDoc.exists) {
            ...
            if (lastMessage == null || lastMessage == 'You matched! Say hello!') {
  ...
  Future<bool> extendMatchExpiration(String matchId, Duration extension) async {
    return await PerformanceMonitor.measure('extend_match_expiration', () async {
      try {
        final matchDoc = await _matchesCollection.doc(matchId).get();
        if (!matchDoc.exists) {
        ...
        final newExpiry = currentExpiry != null 
            ...
            : Timestamp.fromDate(DateTime.now().add(MATCH_EXPIRY_DURATION).add(extension));
        ...
        debugPrint('✅ Extended match $matchId expiration by ${extension.inDays} days');
        return true;
  ...
  Future<void> _performScheduledCleanup() async {
    await PerformanceMonitor.measure('scheduled_cleanup', () async {
      try {
        ...
        debugPrint('✅ Cleanup complete: $archivedCount archived, $skippedCount skipped');
    ...
    return await PerformanceMonitor.measure('restore_archived_match', () async {
      try {
        ...
        for (final userId in userIds) {
          ...
          batch.set(_usersCollection.doc(userId).collection('Matches').doc(otherUserId), {
            'Matches': otherUserId,
            'isRead': false,
...
class MatchExpirationStats {
  ...
  double get messageRate => totalMatches > 0 ? matchesWithMessages / totalMatches : 0.0;
  double get expirationRate => totalMatches > 0 ? expiredMatches / totalMatches : 0.0;
