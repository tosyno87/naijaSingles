...
class PerformanceMonitor {
  ...
  static void _logPerformance(String operationName, int elapsedMs) {
    ...
    if (elapsedMs > threshold) {
      debugPrint('   ⚠️ Exceeded threshold of ${threshold}ms by ${elapsedMs - threshold}ms');
    }
  }
  ...
  static void recordFirestoreOperation(String operationType, {int? readCount, int? writeCount}) {
    ...
    if (writeCount != null) {
      ...
      _operationCounts[writeKey] = (_operationCounts[writeKey] ?? 0) + writeCount;
    }
    
    debugPrint('📊 Firestore $operationType: reads=${readCount ?? 0}, writes=${writeCount ?? 0}');
  }
  ...
  static String generateReport() {
    ...
    for (final entry in sortedStats) {
      ...
      buffer.writeln('   Average: ${stat.averageMs}ms (threshold: ${stat.threshold}ms)');
