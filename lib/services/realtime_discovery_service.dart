...
class RealtimeDiscoveryService {
  ...
  static Stream<List<UserModel>> getUsersStream(UserModel currentUser) {
    try {
      debugPrint('🔍 Starting real-time discovery stream for ${currentUser.name}');
      
      // Build query with basic filters
      Query query = _firestore.collection('users')
          .where('id', isNotEqualTo: currentUser.id) // Exclude current user
          .where('userGender', isEqualTo: currentUser.showGender) // Gender preference
        ...
        for (var doc in snapshot.docs) {
          try {
            ...
            if (user.latitude != null && user.longitude != null &&
              ...
              user.distanceBW = distance.calculateDistance(
  ...
  static Stream<List<UserModel>> getPaginatedUsersStream(
    ...
    try {
      debugPrint('📄 Starting paginated real-time stream (page size: $pageSize)');
  ...
  static Stream<List<UserModel>> getNearbyUsersStream(
    ...
    try {
      ...
      return query.snapshots().asyncMap((snapshot) async {
        ...
        debugPrint('✅ Nearby stream processed: ${users.length} users within ${radiusMiles}mi');
        return users;
      });
    } catch (e) {
  ...
  static Stream<Map<String, dynamic>> getDiscoveryStatsStream(UserModel currentUser) {
    try {
      return _firestore
          .collection('users')
          .doc(currentUser.id)
