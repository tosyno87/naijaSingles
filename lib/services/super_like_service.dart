...
class SuperLikeService {
  ...
  CollectionReference get _superLikesCollection => _firestore.collection('superLikes');
  ...
  Future<SuperLikeResult> sendSuperLike({
    ...
    return await PerformanceMonitor.measure('send_super_like', () async {
      try {
        ...
        if (!canSend.canSend) {
          return SuperLikeResult.failed(canSend.reason ?? 'Cannot send super like');
        }
        
        // Check if already super liked this user
        final existingSuperLike = await _getExistingSuperLike(fromUserId, toUserId);
        if (existingSuperLike != null) {
          return SuperLikeResult.failed('You have already super liked this user');
        }
  ...
  Future<SuperLikeEligibility> canSendSuperLike(String userId) async {
    try {
      ...
      final dailyLimit = isPremium ? PREMIUM_SUPER_LIKES_PER_DAY : FREE_SUPER_LIKES_PER_DAY;
  ...
  Future<List<SuperLike>> getReceivedSuperLikes(String userId) async {
    return await PerformanceMonitor.measure('get_received_super_likes', () async {
      try {
        final querySnapshot = await _superLikesCollection
        ...
        debugPrint('📬 Found ${superLikes.length} super likes for user $userId');
        return superLikes;
  ...
  Future<List<SuperLike>> getSentSuperLikes(String userId) async {
    return await PerformanceMonitor.measure('get_sent_super_likes', () async {
      try {
        ...
        debugPrint('📤 Found ${superLikes.length} sent super likes for user $userId');
        return superLikes;
    ...
    return await PerformanceMonitor.measure('respond_to_super_like', () async {
      try {
        ...
        if (isLike) {
          ...
          final matchResult = await _matchService.handleLike(respondingUserId, superLike.fromUserId);
          
          if (matchResult.isSuccess && matchResult.isMatch) {
            debugPrint('🎉 Super like resulted in match: ${matchResult.matchId}');
            ...
            await _likesCollection.doc('${respondingUserId}_likes_${superLike.fromUserId}').set({
  ...
  Future<String?> _createSuperLike({
    ...
    try {
      ...
      await superLikeRef.set({
        ...
        'fromUserImageUrl': (fromUserData['imageUrl'] as List?)?.isNotEmpty == true 
        ...
        'isActive': true,
        'responded': false,
        'highlightUntil': Timestamp.fromDate(DateTime.now().add(SUPER_LIKE_HIGHLIGHT_DURATION)),
      });
  ...
  Future<String?> _checkForInstantMatch(String fromUserId, String toUserId) async {
    try {
      // Check if recipient has already liked the sender
      final reverseLikeDoc = await _likesCollection.doc('${toUserId}_likes_${fromUserId}').get();
      
      if (reverseLikeDoc.exists) {
        // Create match
        final matchResult = await _matchService.handleLike(fromUserId, toUserId);
  ...
  Future<SuperLike?> _getExistingSuperLike(String fromUserId, String toUserId) async {
    try {
      final querySnapshot = await _superLikesCollection
  ...
  Future<bool> _hasAlreadyLiked(String fromUserId, String toUserId) async {
    try {
      final likeDoc = await _likesCollection.doc('${fromUserId}_likes_${toUserId}').get();
  ...
  Future<SuperLikeStats> getSuperLikeStats(String userId) async {
    try {
      ...
      return SuperLikeStats(
        ...
        dailyLimit: isPremium ? PREMIUM_SUPER_LIKES_PER_DAY : FREE_SUPER_LIKES_PER_DAY,
  ...
  Future<void> cleanupExpiredHighlights() async {
    try {
      ...
      if (expiredQuery.docs.isNotEmpty) {
        await batch.commit();
        debugPrint('🧹 Cleaned up ${expiredQuery.docs.length} expired super like highlights');
      }
...
class SuperLike {
  ...
  bool get isHighlighted => highlightUntil != null && DateTime.now().isBefore(highlightUntil!);
