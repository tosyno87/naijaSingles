...
class UndoService {
  ...
  CollectionReference get _swipeHistoryCollection => _firestore.collection('swipeHistory');
  ...
  Future<void> recordSwipeAction({
    ...
    try {
      ...
      debugPrint('📝 Recorded swipe: $userId → $targetUserId (${direction.name})');
  ...
  Future<bool> canUndoLastSwipe(String userId) async {
    try {
      ...
      if (undoCount >= limit) {
        debugPrint('⚠️ User $userId has reached daily undo limit ($undoCount/$limit)');
        return false;
      }
    ...
    return await PerformanceMonitor.measure('undo_last_swipe', () async {
      try {
        ...
        if (!await canUndoLastSwipe(userId)) {
          return UndoResult.failed('Cannot undo: outside time window or limit reached');
        }
        ...
        if (success) {
          ...
          debugPrint('✅ Successfully undid swipe: ${lastSwipe.userId} → ${lastSwipe.targetUserId}');
  ...
  Future<bool> _performUndo(SwipeAction swipeAction) async {
    try {
      ...
      final historyQuery = await _swipeHistoryCollection
          ...
          .where('timestamp', isEqualTo: Timestamp.fromDate(swipeAction.timestamp))
          .limit(1)
          .get();
  ...
  Future<void> _undoRightSwipe(SwipeAction swipeAction, WriteBatch batch) async {
    ...
    if (swipeAction.matchId != null) {
      ...
      batch.delete(_usersCollection.doc(userId).collection('Matches').doc(targetUserId));
      batch.delete(_usersCollection.doc(targetUserId).collection('Matches').doc(userId));
      
      debugPrint('🔥 Undoing match: ${swipeAction.matchId}');
    }
    
    // Remove from checked users so they can see each other again
    batch.delete(_usersCollection.doc(userId).collection('CheckedUser').doc(targetUserId));
  ...
  Future<int> getDailyUndoCount(String userId) async {
    try {
      ...
      final querySnapshot = await _swipeHistoryCollection
          ...
          .where('undoneAt', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfDay))
          .get();
  ...
  Future<void> clearExpiredHistory() async {
    try {
      ...
      if (expiredQuery.docs.isNotEmpty) {
        await batch.commit();
        debugPrint('🗑️ Cleared ${expiredQuery.docs.length} expired swipe history entries');
      }
...
class UndoStats {
  ...
  int get remainingUndos => (dailyUndoLimit - dailyUndoCount).clamp(0, dailyUndoLimit);
  double get usagePercentage => dailyUndoLimit > 0 ? dailyUndoCount / dailyUndoLimit : 0.0;
