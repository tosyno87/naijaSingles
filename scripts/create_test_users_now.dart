...
// Firebase configuration (standalone - no Flutter dependencies)
// Using production values from naijasingles-74a75 project
...
// Constants
const Duration _rateLimitDelay = Duration(milliseconds: 500);
  ...
  for (final cityEntry in cities.entries) {
    ...
    for (int i = 0; i < 10; i++) {
      try {
        ...
        totalCreated++;
        print('  ✅ Male ${i + 1}/10');
        await Future.delayed(_rateLimitDelay); // Rate limit delay
      } catch (e) {
        totalFailed++;
...
Map<String, dynamic> generateUserData(
  ...
  final lat =
      (city['lat'] as num).toDouble() + (random.nextDouble() - 0.5) * 0.1;
  final lng =
      (city['lng'] as num).toDouble() + (random.nextDouble() - 0.5) * 0.1;
