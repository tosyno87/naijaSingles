...
void main() {
  ...
  group('Performance Tests', () {
    test('Matching performance under load', () async {
      ...
      // Create test users
      final users = List.generate(100, (index) => UserModel(
        id: 'user_$index',
        name: 'User $index',
        age: 20 + (index % 30),
        nationality: ['Nigeria', 'Ghana', 'Ethiopia', 'Kenya'][index % 4],
      ...
      expect(matchingDuration.inMilliseconds, lessThan(1000)); // Should complete in < 1 second
      
      final totalDuration = DateTime.now().difference(startTime);
      expect(totalDuration.inMilliseconds, lessThan(2000)); // Total test should complete in < 2 seconds
      
      print('✅ Matching performance: ${matchingDuration.inMilliseconds}ms for ${users.length} users');
    });
    ...
    test('Photo upload performance', () async {
      ...
      for (final size in photoSizes) {
        ...
        await Future.delayed(Duration(milliseconds: size ~/ (1024 * 100))); // Simulate upload time
        ...
        print('✅ Photo upload performance (${size ~/ (1024 * 1024)}MB): ${uploadDuration.inMilliseconds}ms');
      }
    });
    ...
    test('Database query optimization', () async {
      ...
      for (final queryType in queryTypes) {
        ...
        await Future.delayed(Duration(milliseconds: 50)); // Simulate query time
        ...
        print('✅ Database query performance ($queryType): ${queryDuration.inMilliseconds}ms');
      }
    });
    ...
    test('Memory usage optimization', () async {
      ...
      final largeUserList = List.generate(1000, (index) => UserModel(
        id: 'user_$index',
        name: 'User $index',
        age: 20 + (index % 30),
        nationality: 'Nigeria',
        tribe: 'Yoruba',
        imageUrl: List.generate(5, (i) => 'https://example.com/photo_${index}_$i.jpg'),
      ));
      
      // Process users
      final processedUsers = largeUserList.map((user) {
        return UserModel(
          id: user.id,
          name: user.name,
          age: user.age,
      ...
      print('✅ Memory usage: ${memoryIncrease ~/ (1024 * 1024)}MB increase for 1000 users');
    });
    ...
    test('Network performance across US regions', () async {
      ...
      for (final region in usRegions) {
        ...
        print('✅ Network performance (${region['name']}): ${networkDuration.inMilliseconds}ms');
      }
    });
    ...
    test('Real-time messaging latency', () async {
      ...
      for (final size in messageSizes) {
        ...
        await Future.delayed(Duration(milliseconds: size ~/ 100)); // Simulate send time
        ...
        print('✅ Message latency (${size} chars): ${messageDuration.inMilliseconds}ms');
      }
    });
    ...
    test('Cultural filtering performance', () async {
      ...
      final users = List.generate(500, (index) => UserModel(
        id: 'user_$index',
        name: 'User $index',
      ...
      print('✅ Cultural filtering performance: ${filteringDuration.inMilliseconds}ms for ${users.length} users');
    });
    ...
    test('Background task performance', () async {
      ...
      for (final task in backgroundTasks) {
        ...
        await Future.delayed(Duration(milliseconds: 100)); // Simulate task time
